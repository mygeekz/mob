...
const PhoneInstallmentSalesReportPage: React.FC = () => {
    ...
    const fetchReport = async () => {
        ...
        try {
            const response = await apiFetch(`/api/reports/phone-installment-sales?fromDate=${fromDateShamsi}&toDate=${toDateShamsi}`);
            const result = await response.json();
            if (!response.ok || !result.success) {
                throw new Error(result.message || 'خطا در دریافت گزارش فروش اقساطی موبایل');
            }
    ...
    const columns = useMemo(() => [
        columnHelper.accessor('dateCreated', { header: 'تاریخ ثبت', cell: info => formatIsoToShamsiDateTime(info.getValue(), 'jD jMMMM jYYYY - HH:mm') }),
        ...
        columnHelper.accessor('actualSalePrice', { header: 'مبلغ فروش اقساطی', cell: info => formatSimplePrice(info.getValue()) }),
        columnHelper.accessor('totalProfit', { header: 'سود کل', cell: info => formatPrice(info.getValue()) }),
    ], []);
    ...
    const totalProfit = table.getFilteredRowModel().rows.reduce((sum, row) => sum + row.original.totalProfit, 0);
    const totalSaleValue = table.getFilteredRowModel().rows.reduce((sum, row) => sum + row.original.actualSalePrice, 0);
    ...
    return (
        <div className="space-y-6 text-right" dir="rtl">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b pb-3">گزارش فروش اقساطی موبایل</h2>
                <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="bg-purple-50 p-4 rounded-lg text-center">
                        <p className="text-sm text-purple-800">مجموع سود (برای نتایج فیلتر شده)</p>
                        <p className="text-2xl font-bold text-purple-700">{formatPrice(totalProfit)}</p>
                    </div>
                     <div className="bg-orange-50 p-4 rounded-lg text-center">
                        <p className="text-sm text-orange-800">مجموع ارزش فروش (برای نتایج فیلتر شده)</p>
                        <p className="text-2xl font-bold text-orange-700">{formatSimplePrice(totalSaleValue)}</p>
                    </div>
                </div>
