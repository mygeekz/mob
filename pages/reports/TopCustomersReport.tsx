...
const TopCustomersReportPage: React.FC = () => {
  ...
  const fetchTopCustomers = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/top-customers?fromDate=${fromDateShamsi}&toDate=${toDateShamsi}`, { headers: getAuthHeaders(token) });
  ...
  const columns = useMemo(() => [
      ...
      columnHelper.accessor('totalSpent', { header: 'مجموع خرید', cell: info => <span className="font-semibold text-green-700">{formatPrice(info.getValue())}</span> }),
      ...
      columnHelper.accessor('customerId', {
          ...
          cell: info => <Link to={`/customers/${info.getValue()}`} className="text-indigo-600 hover:text-indigo-800 transition-colors">مشاهده جزئیات</Link>
      }),
  ], []);
